...
The template i used, used Tinacms, I just went with it for my cms, configuring its environment variables  with Astro posed an issue, the method to use the variable  provided on the documentation, wasn't  working so after much research i found an answer. 

Also building the tinacms kept failing in production, after searching there discord and rereading carefully I realised i want using the latest version of the package had to update that. Typical npm always giving headaches with their dependency tree.

So yeah that's all the process that went into building the blog, nothing to fancy, but I am celebrating a little win. Looking forward to getting better at writing.
